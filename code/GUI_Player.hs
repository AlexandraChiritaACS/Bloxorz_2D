...
--METHODS--
-----------
createPlayerWindow :: IO()
createPlayerWindow = do
    --Start music game
    ...
    --Draw level (map & block) on catch event
    windowOnPaintRaw p (drawLevel varGame varPositions)
    --Get event key pressed
    windowOnKeyChar p (onKey p f varGame varPositions)
    --Show window
    windowShow f
    windowRaise f
    return ()
    where
        ...
        onKey p f varGame varPositions keyboard =
            case keyKey keyboard of
                Wx.KeyUp -> moveBlock p varGame varPositions Mv.KeyUp
                Wx.KeyLeft -> moveBlock p varGame varPositions Mv.KeyLeft
                Wx.KeyDown -> moveBlock p varGame varPositions Mv.KeyDown
                Wx.KeyRight -> moveBlock p varGame varPositions Mv.KeyRight
                Wx.KeyChar 'r' -> moveBlock p varGame varPositions Mv.KeyRestart
                Wx.KeyEscape -> endGame f
                other -> skipCurrentEvent
        --MODIFY BLOCK POSITION AFTER PRESS KEY
        moveBlock :: Window a -> Var Game -> Var [[Rect]] -> Movement -> IO()
        moveBlock p varGame varPositions movement = do
            game <- varGet varGame
            let newBlock = createNextPosition (level game) (piece game) movement
            --RESTART LEVEL
            if (state newBlock) == OutOfBorders || (state newBlock) == Falling
                then do
                    restartLevel
                    varSet varGame (createLevel (nLevel game))
                    refreshBoard p
            --NEXT LEVEL
            else if (state newBlock) == Winning && (((nLevel game)+1) < (length maps))
                then do
                    ...
                    nextLevel
            --FINISH GAME
            else if (state newBlock) == Winning && ((nLevel game) == ((length maps)-1))
                then do
                    winGame
            -- PLAYING
            else do
                refreshBlock p newBlock varGame varPositions
        --REFRESH BLOCK
        refreshBlock :: Window a -> Piece -> Var Game -> Var [[Rect]] -> IO()
        refreshBlock p newBlock varGame varPositions = do
            game <- varGet varGame
